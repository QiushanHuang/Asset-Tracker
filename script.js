...
(function installAssetTrackerHost() {
    ...
    const nativeHandler = window.webkit &&
        ...
        window.webkit.messageHandlers.assetTrackerHost;

    const pendingRequests = new Map();
    let requestSequence = 0;

    const host = {
        isNative: !!nativeHandler,
        ...
        invoke(type, payload = {}) {
            ...
            const id = `assetTrackerReq-${Date.now()}-${++requestSequence}`;
            ...
            return new Promise((resolve, reject) => {
                ...
                nativeHandler.postMessage({ id, type, payload });
            });
        },
...
class AssetTrackerFileAdapter {
    async saveFile({ suggestedName, mimeType = 'application/octet-stream', text, encoding = 'text' }) {
        if (window.AssetTrackerHost?.isAvailable?.()) {
            return window.AssetTrackerHost.invoke('file.saveExport', {
                suggestedName,
                mime: mimeType,
                text,
                encoding
        ...
        const payload = encoding === 'base64'
            ? this._decodeBase64ToBytes(text)
    ...
    async openImport({ acceptedTypes = '.xlsx,.csv,.json', fileInputId, readAs = 'binary' }) {
        if (window.AssetTrackerHost?.isAvailable?.()) {
            return window.AssetTrackerHost.invoke('file.openImport', {
                acceptedTypes,
                readAs
            });
        }
        ...
        return new Promise((resolve, reject) => {
            ...
            const onChange = () => {
                ...
                const readMethod = readAs === 'binary' ? 'readAsBinaryString' : 'readAsText';
                const readBinary = () => {
                    const reader = new FileReader();
                    ...
                    reader.onload = (e) => {
                        ...
                        resolve({
                            ...
                            text: rawText,
                            mime: file.type || 'application/octet-stream',
                            encoding: readAs === 'binary' ? 'binary' : 'text'
                        });
                    };
    ...
    async revealDataFolder() {
        if (window.AssetTrackerHost?.isAvailable?.()) {
            return window.AssetTrackerHost.invoke('file.revealFolder');
        }
    ...
    normalizeImportedContent(text, encoding = 'text', { output = 'binary' } = {}) {
        ...
        const decodedText = this.decodeBase64ToText(text);
        return output === 'binary' ? decodedText.binary : decodedText.utf8;
    }

    _decodeBase64ToBytes(base64String) {
        if (!base64String) {
    ...
    decodeBase64ToText(base64String) {
        const bytes = this._decodeBase64ToBytes(base64String);
        const binary = Array.from(bytes, byte => String.fromCharCode(byte)).join('');
        const utf8Text = typeof TextDecoder === 'undefined'
...
class AssetTrackerStorageAdapter {
    constructor() {
        this.stateHash = '';
        this.storageKey = 'assetTrackerData';
...
class AssetTracker {
    constructor() {
        ...
        this.charts = {};
        this.storageAdapter = new AssetTrackerStorageAdapter();
        this.fileAdapter = new AssetTrackerFileAdapter();
        this.storageMeta = {
            stateHash: '',
    ...
    refreshStorageDisplay() {
        const storageNode = document.getElementById('storage-path-display');
        if (!storageNode) {
            return;
        }

        storageNode.textContent = this.storageAdapter.supportsNative
            ? `当前数据存储：${this.storageMode}（${this.storageMeta.storagePath || '未设置路径'}）`
            : '当前数据存储：浏览器本地存储（localStorage）';
    }

    normalizeLoadedData(state) {
        const defaultState = this.getDefaultState();
        const loaded = state && typeof state === 'object' ? state : {};

        const safeSettings = loaded.settings && typeof loaded.settings === 'object'
            ? loaded.settings
            : {};
        ...
        const normalizedSettings = {
            ...
            exchangeRates: {
                ...
                ...(safeSettings.exchangeRates && typeof safeSettings.exchangeRates === 'object' ? safeSettings.exchangeRates : {})
            },
            autoBackup: Boolean(safeSettings.autoBackup ?? defaultSettings.autoBackup),
            backupInterval: Number.isFinite(Number(safeSettings.backupInterval))
                ? Math.max(1, Number(safeSettings.backupInterval))
                : defaultSettings.backupInterval
        };
        ...
        return {
            ...
            purposeCategories: Array.isArray(loaded.purposeCategories)
                ? loaded.purposeCategories
                : defaultState.purposeCategories,
            initialAssets: Array.isArray(loaded.initialAssets) ? loaded.initialAssets : defaultState.initialAssets,
            transactionTemplates: Array.isArray(loaded.transactionTemplates)
                ? loaded.transactionTemplates
                : defaultState.transactionTemplates,
            memo: typeof loaded.memo === 'string' ? loaded.memo : '',
    ...
    getBookExportPackage() {
        return {
            format: ASSET_BOOK_FORMAT,
            formatVersion: ASSET_BOOK_FORMAT_VERSION,
            schemaVersion: ASSET_BOOK_SCHEMA_VERSION,
            exportedAt: new Date().toISOString(),
            source: this.storageAdapter.supportsNative ? 'macos-app' : 'web',
            payload: this.data
        };
    }
    ...
    async setupAutoBackup() {
        ...
        const fallbackKey = 'assetTrackerLastBackupTime';
        const lastBackup = this.storageAdapter.supportsNative
            ? null
    ...
    async saveBackupSettings() {
        ...
        if (!autoBackupCheckbox || !intervalInput) {
            return;
        }

        const intervalHours = parseInt(intervalInput.value, 10);
        if (Number.isNaN(intervalHours) || intervalHours < 1 || intervalHours > 168) {
            this.showMessage('备份间隔应为 1-168 小时', 'error');
            return;
        }
    ...
    exportToExcel() {
        ...
        this.fileAdapter.saveFile({
            ...
            this.showMessage(`导出失败：${error?.message || '请重试'}`, 'error');
        });
    }
    ...
    exportToJSON() {
        const packageData = this.getBookExportPackage();
    ...
    downloadTemplate() {
        ...
        const outputFileName = '资产记录导入模板.xlsx';
        const templateBinary = XLSX.write(workbook, {
            bookType: 'xlsx',
            type: this.storageAdapter.supportsNative ? 'base64' : 'array'
        });
        ...
        if (this.storageAdapter.supportsNative) {
            this.fileAdapter.saveFile({
                ...
                this.showMessage(`模板下载失败：${error?.message || '请重试'}`, 'error');
            });
        } else {
            const blob = new Blob([templateBinary], { type: 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet' });
            const url = URL.createObjectURL(blob);
            const link = document.createElement('a');
            link.href = url;
    ...
    async resetToDefaultData() {
        if (!window.confirm('恢复默认账本将清空当前数据，是否继续？')) {
            return;
        }
    ...
    initializeSettings() {
        ...
        if (autoBackupCheckbox) {
            autoBackupCheckbox.checked = !!this.data.settings.autoBackup;
        }

        if (intervalInput) {
            intervalInput.value = Number.isFinite(this.data.settings.backupInterval)
                ? this.data.settings.backupInterval
                : 24;
        }
    ...
    saveCurrencySettings() {
        ...
        this.data.settings.exchangeRates = newExchangeRates;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    window.assetTracker.initialize().then(() => {
        window.assetTracker.initialized = true;
    }).catch(() => {
        window.assetTracker.initialized = false;
        window.assetTracker.showMessage('应用初始化失败，请检查本地文件访问权限。', 'error');
    });
});
