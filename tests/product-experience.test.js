...
test('deleting a history-only WeChat import keeps the current account balance unchanged',t=>{const {tracker}=fixture(t);tracker.data.categories={wallet:{id:'wallet',name:'微信零钱',currency:'CNY',balance:100}};tracker.data.transactions=[{id:'wechat:test',date:'2026-09-19',category:'微信零钱',accountId:'wallet',amount:-10,currency:'CNY',description:'历史流水',wechat:{balanceMode:'history'}}];tracker.persistData=async()=>{};tracker.renderTransactions=tracker.renderCategories=tracker.updateDashboard=()=>{};tracker.deleteTransaction('wechat:test');assert.equal(tracker.data.categories.wallet.balance,100);assert.equal(tracker.data.transactions.length,0);});
